...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const t = Date.now();
        const teamRes = await fetch(`/api/data/team/${teamId}?t=${t}`, {
          cache: "no-store",
        });
        if (teamRes.ok) setUserTeam(await teamRes.json());

        const allTeamsRes = await fetch(`/api/data/teams?t=${t}`, {
          cache: "no-store",
        });
        if (allTeamsRes.ok) setAllTeams(await allTeamsRes.json());

        let plotsData: any[] = [];
        const plotsRes = await fetch(`/api/data/plots?t=${t}`, {
          cache: "no-store",
        });
        if (plotsRes.ok) {
        ...
        const stateRes = await fetch(`/api/admin/state?t=${t}`, {
          cache: "no-store",
        });
        if (stateRes.ok) {
          ...
          if (stateData.current_plot) {
            const fullPlot = plotsData.find(
              (p: any) => p.number === stateData.current_plot.number,
            );
          ...
          if (
            stateData.current_policy_deltas &&
            Object.keys(stateData.current_policy_deltas).length > 0
          ) {
        ...
        const bidsRes = await fetch(`/api/data/bids/recent?t=${t}`, {
          cache: "no-store",
        });
        if (bidsRes.ok) setRecentBids(await bidsRes.json());

        const offersRes = await fetch(`/api/data/rebid-offers?t=${t}`, {
          cache: "no-store",
        });
        if (offersRes.ok) setRebidOffers(await offersRes.json());

        const soldOffersRes = await fetch(
          `/api/data/rebid-offers-sold?t=${t}`,
          { cache: "no-store" },
        );
  ...
  useEffect(() => {
        ...
        setPlots((prev) =>
          prev.map((p) => {
            ...
            if (p.status === "active") {
              return {
                ...p,
                status: data.status === "reversed" ? "pending" : "sold",
              };
            }
            return p;
          }),
    ...
    const handleTeamUpdate = (data: any) => {
      ...
      const myTeamId = userTeamRef.current
        ? String(userTeamRef.current.id)
        : null;
    ...
    // Rebid Listeners
    socket.on("rebid_phase_update", (data: any) =>
  ...
  const getRoundLabel = (round: number) => {
    switch (round) {
      ...
      case 2:
        return "Round 2 — Adjustments";
      case 3:
        return "Round 3 — Adjustments";
      case 4:
        return "Round 4 — Final Bidding";
      case 5:
        return "Round 5 — Adjustments";
      case 6:
        return "Round 6 — Final Adjustments";
      default:
        return `Round ${round}`;
    }
  };
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-3 gap-2 border-b-4 border-[var(--color-border)] pb-3 bg-[var(--color-bg)] shrink-0">
          <div>
            ...
            <div className="flex items-center gap-3 text-xs font-bold uppercase mt-1">
              <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 border-2 border-[var(--color-border)] ${isConnected ? "bg-[var(--color-success)]" : "bg-[var(--color-danger)]"}`}
              >
                <span
                  className={`w-2 h-2 border border-[var(--color-border)] ${isConnected ? "bg-[var(--color-success)]" : "bg-[var(--color-danger)]"}`}
                />
          ...
          <div className="flex flex-wrap gap-2 sm:gap-4 items-center justify-end">
            {(() => {
              ...
              return (
                <div className="flex neo-border  text-[var(--color-bg)] shadow-[4px_4px_0_var(--neo-shadow-color)] overflow-hidden">
                  ...
                  <div className="flex flex-col justify-center  bg-[var(--color-plots)] items-center px-3 sm:px-4 py-2 border-r-2 border-[var(--color-bg)]/15">
                    <p className="text-[8px] sm:text-[10px] font-black uppercase opacity-60 tracking-wider">
                      Plots
                    </p>
                    <p className="font-mono font-black text-sm sm:text-lg leading-tight text-[var(--color-bg)]">
            ...
            <div className="flex items-center gap-2 neo-border border-[var(--color-primary)] px-3 py-2 bg-[var(--color-primary)] text-[var(--color-bg)] shadow-[4px_4px_0_var(--neo-shadow-color)]">
              ...
              <span className="font-black text-sm uppercase hidden sm:inline">
                {userTeam.name}
              </span>
            </div>
                ...
                {allTeams
                  .map((t) => {
                    const remainingBudget =
                      Number(t.budget) - Number(t.spent || 0);
                    ...
                    const propertyValue = teamPlots.reduce(
                      (sum, p) =>
                        sum +
                        ((Number(p.current_bid) ||
                          Number(p.total_plot_price) ||
                          0) +
                          Number(p.round_adjustment || 0)),
                      0,
                    );
            ...
            <NeoCard className="p-0 overflow-hidden relative bg-[var(--color-bg)] w-full max-w-3xl aspect-square lg:aspect-auto lg:h-full">
              <div className="absolute inset-0 flex items-center justify-center z-10 bg-black/30">
                <div className="text-center">
                  <MapPin
                    size={64}
                    className="mx-auto mb-3 text-[var(--color-bg)] opacity-70"
                  />
            ...
            <div className="lg:col-span-8 flex flex-col gap-3 min-h-0">
              <NeoCard className="flex flex-col shrink-0 lg:flex-none border-4 border-[var(--color-border)] bg-[var(--color-primary)] text-[var(--color-bg)] shadow-[8px_8px_0_var(--neo-shadow-color)]">
                <h2 className="text-sm font-black uppercase mb-2 opacity-80 flex items-center gap-2">
                  <AlertTriangle size={16} /> Active Policy (Round{" "}
                  {currentRound})
                </h2>
              ...
              <NeoCard className="h-full overflow-hidden flex flex-col bg-[var(--color-bg)]">
                ...
                <div className="space-y-2 overflow-y-auto pr-1 flex-1 min-h-0">
                  {[...allTeams]
                    .map((team) => {
                      const remaining =
                        Number(team.budget) - Number(team.spent || 0);
                      const portfolioValue = plots
                        .filter(
                          (p) =>
                    ...
                    .map((team, idx) => {
                      return (
                        <div
                          key={team.id}
                          className="flex flex-col bg-[var(--color-surface)] p-2 neo-border"
                        >
                          <div className="flex justify-between items-center mb-1">
                            <span className="font-black text-sm uppercase">
                              #{idx + 1} {team.name}
                            </span>
                            <span className="font-bold text-xs text-[var(--color-success)] tracking-wider">
                              ★ {team.plotsWon} WON
                            </span>
                          </div>
                ...
                <div className="overflow-y-auto pr-1 flex-1 space-y-2">
                  ...
                  {plots
                    ...
                    .map((plot) => {
                      ...
                      const maxAllowed = currentValue * 1.1;
                      const minAllowed = currentValue;
                      const activeOffer = rebidOffers.find(
                        (o) =>
                          o.plot_number === plot.number &&
                          o.status === "active",
                      );
                      ...
                      return (
                        <div
                          key={plot.number}
                          className="neo-border p-3 bg-[var(--color-surface)]"
                        >
                          ...
                          {activeOffer ? (
                            <div className="bg-[var(--color-surface)] p-2 neo-border flex justify-between items-center text-[var(--color-text)] gap-2">
                              <div className="flex flex-col">
                                <span className="text-xs font-bold uppercase">
                                  Listed for ₹
                                  {Number(
                                    activeOffer.asking_price,
                                  ).toLocaleString("en-IN")}
                                </span>
                                <span className="text-[10px] font-black animate-pulse">
                                  WAITING FOR BUYER...
                                </span>
                              </div>
                              <NeoButton
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <p className="text-[10px] font-bold opacity-50 px-1">
                                Min: ₹
                                {Math.floor(minAllowed).toLocaleString("en-IN")}{" "}
                                — Max: ₹
                                {Math.floor(maxAllowed).toLocaleString("en-IN")}
                              </p>
                            </div>
                          )}
                        </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-3 lg:min-h-0 lg:overflow-y-auto">
              ...
              {currentPlot ? (
                <NeoCard>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {Number(currentPlot.round_adjustment) !== 0 &&
                      currentRound !== 4 && (
                        <div
                          className={`neo-border p-3 ${currentPlot.round_adjustment > 0 ? "bg-[var(--color-success)]/20" : "bg-[var(--color-danger)]/20"}`}
                        >
                          <p className="text-xs font-bold uppercase text-[var(--color-text)] opacity-50">
                            Round Adjustment
                          </p>
                          <p
                            className={`font-black text-lg ${currentPlot.round_adjustment > 0 ? "text-[var(--color-success)]" : "text-[var(--color-danger)]"}`}
                          >
                ...
                <NeoCard className="flex flex-col items-center justify-center min-h-[200px] text-center bg-[var(--color-bg)] border-dashed">
                  <p className="font-bold text-xl uppercase text-[var(--color-text)] opacity-40">
                    Waiting for plot...
                  </p>
                </NeoCard>
              )}
