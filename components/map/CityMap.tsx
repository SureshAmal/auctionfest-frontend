...
export default function CityMap({
  ...
  const [highlightMode, setHighlightMode] = useState<"changes" | "types">(
    "changes",
  );
    ...
    <div className="relative w-full h-full overflow-hidden bg-[var(--color-bg)]">
      ...
      >
        ...
        {currentRound > 1 ? (
          <City2SVG
            ref={svgRef}
            currentPlotNumber={currentPlotNumber?.toString() || ""}
        ...
        {hoveredPlot && (
          ...
          >
            <div className="flex flex-col items-center">
              <span className="font-black text-lg mb-1">
              ...
              {isRound1 ? (
                <>
                  {hoveredPlot.winner_team_id && (
                    <div className="flex items-center gap-2 mb-1">
                      <span className="opacity-70 text-xs font-bold uppercase">
                        Team:
                      </span>
                      <span className="text-[var(--color-primary)] font-bold">
                        {allTeams.find(
                          (t) => t.id === hoveredPlot.winner_team_id,
                        )?.name || "—"}
                      </span>
                    </div>
                  )}
                ...
                <>
                  ...
                  {hoveredPlot.winner_team_id && (
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[var(--color-primary)] font-bold">
                        {allTeams.find(
                          (t) => t.id === hoveredPlot.winner_team_id,
                        )?.name || hoveredPlot.winner_team_id}
                      </span>
                    </div>
                  )}
                  {/* Area */}
                  {(hoveredPlot.total_area || hoveredPlot.actual_area) && (
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-mono font-bold text-xs">
                        {hoveredPlot.total_area} sqft{" "}
                        <span className="opacity-50">
                          (Actual: {hoveredPlot.actual_area} sqft)
                        </span>
                      </span>
                    </div>
                  )}
                  ...
                  {Number(hoveredPlot.round_adjustment || 0) !== 0 && (
                    <>
                      <div
                        className={`flex items-center gap-2 mt-1 px-2 py-0.5 border ${Number(hoveredPlot.round_adjustment) > 0 ? "border-green-400/50 text-green-400" : "border-red-400/50 text-red-400"}`}
                      >
                        ...
                        <span className="font-mono font-bold text-xs">
                          ...
                          {Number(hoveredPlot.round_adjustment).toLocaleString(
                            "en-IN",
                          )}
                        </span>
                      </div>
                  ...
                  {recentAdjustments[hoveredPlot.number] !== undefined && (
                    <div
                      className={`flex items-center gap-2 mt-1 px-2 py-0.5 border-2 ${recentAdjustments[hoveredPlot.number] > 0 ? "border-orange-400 text-orange-300" : "border-red-400 text-red-300"}`}
                    >
                      <span className="text-[10px] font-black uppercase">
                        This Policy:
                      </span>
                      <span className="font-mono font-black text-sm">
                        {recentAdjustments[hoveredPlot.number] > 0 ? "+" : ""}₹
                        {recentAdjustments[hoveredPlot.number].toLocaleString(
                          "en-IN",
                        )}
                      </span>
                    </div>
        ...
        {isPolicyRound && (
          <>
            ...
            <div
              className="absolute bottom-2 left-2 right-2 pointer-events-auto backdrop-blur-sm px-3 py-1.5 z-50 overflow-x-auto legend-scroll"
              style={{
                backgroundColor: "var(--color-surface)",
                ...
                <div className="flex items-center gap-3 overflow-x-auto legend-scroll">
                  {Array.from(
                    new Set(
                      plots
                        .map((p) =>
                          (p.plot_type || p.type || "").trim().toUpperCase(),
                        )
                        .filter(Boolean),
                    ),
                  ).map((typeStr) => {
